...
if len(sys.argv) < 1:
    ...
    print("     Returns - Script saves a shade table to the current directory.")
    sys.exit(1)
...
def raytrace(dem,elev,azim):
    ...
    if (elev == 0):
        ...
        los=lineofsight(dem.flatten(),elev,azim,los)
    return los.T
...
for i in range(0,len(RMSmult)):
    ...
    slope,azim=gradSlopeAspect(dem)
...
np.save('./out/total_shadow_fraction.npy', tot_shade)
np.save('./out/total_bin_population_4D.npy', bin_pop)
np.save('./out/shade_bin_population_4D.npy', shade_pop)
np.save('./out/shade_lookup_4D.npy', out)
