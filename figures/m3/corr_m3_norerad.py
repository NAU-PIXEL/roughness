...
# Set params
kwin = {"dask": True, "chunks": {"band": None, "y": 20, "x": None}}
kwre = {
    "rms": 15,
    "emissivity": 0.96,
    "rerad": False,
...
outdir = "/work/team_moon/data/comparisons/m3_bandfield_refl/220705/no_rerad/"
...
for k, v in sprio.items():
    ext, *basenames = v
    for basename in basenames:
        fsave = outdir / f"{basename}.nc"
        if fsave.exists():
            print(f"Skipping... {fsave.as_posix()} already exists")
            continue
        ...
        out.to_netcdf(fsave)
        print(f"Saved {basename}.nc to {outdir.as_posix()}")
