...
def make_zsurf(m, n, H, Lx, qr, noiseflag=0):
    """
    ...
    Parameters
    ----------
    m: Size in first dimension.
    n: Size in second dimension.
    H: Hurst exponent.
    Lx: Image width (m).
    ...
    """
    # Use modulo to make sure the dimensions are even.
    # If they aren't, subtract 1. Even is easier for symmetry.
    if m % 2 != 0:
        m = m - 1
    ...
    # Detemine image x/y lengths. This implementation is finite but can be
    ...
    # Make a 2D PSD matrix of all zeros and fill it with the polar rho values
    ...
    # The center of the PSD is the mean. We set this to 0. This is important
    # later when we apply conjugate symmetry.
    ...
    qrmin = np.log10(
        np.sqrt((((2 * np.pi) / Lx) ** 2 + ((2 * np.pi) / Ly) ** 2))
    )
    ...
    # Loop through and find where the rho is ~= the q and calculate the mean of
    # the 2D PSD (Cq) at those locations.
    for j in range(0, len(q)):
        C_AVE[j] = np.nanmean(
            Cq[np.logical_and(rhof > q[j], rhof <= (q[j] + 1))]
        )
    ...
    # This section is transposing certain parts of the magnitude and phase to
    # satisfy conjugate symmetry for input to ifft
    ...
    Bq[0, 1 : int(m / 2)] = np.rot90(
        Bq[0, int(m / 2 + 1) : m].reshape(-1, 1), 2
    ).reshape(-1)
    Bq[int(n / 2 + 1) : n, 0] = np.rot90(
        Bq[1 : int(n / 2), 0].reshape(-1, 1), 2
    ).reshape(-1)
    Bq[int(n / 2 + 1) : n, int(m / 2)] = np.rot90(
        Bq[1 : int(n / 2), int(m / 2)].reshape(-1, 1), 2
    ).reshape(-1)

    # Here is where we introduce the random gaussian portion.
    # I put this flag here so that I can load in a noise array and test the
    # output of notebook and function.
    if noiseflag == 1:
        noisepath = "./zsurf_noise.tmp"
        noise = np.fromfile(noisepath, dtype="double").reshape(m, n)
    else:
    ...
    Phi[0, 1 : int(m / 2)] = -np.rot90(
        Phi[0, int(m / 2 + 1) : m].reshape(-1, 1), 2
    ).reshape(-1)
    Phi[int(n / 2 + 1) : n, 0] = -np.rot90(
        Phi[1 : int(n / 2), 0].reshape(-1, 1), 2
    ).reshape(-1)
    Phi[int(n / 2 + 1) : n, int(m / 2)] = -np.rot90(
        Phi[1 : int(n / 2), int(m / 2)].reshape(-1, 1), 2
    ).reshape(-1)
    ...
    # But it won't do that because of rounding so we use real_if_close
    # (imaginary is tiny).
...
def gradSlope(dem):
    """
    Return the slope of a dem. Slope is derived using NumPy gradient.
    Slope is determined using methods described in this paper:
    ...
    Returns
    -------
    slope (2D array): A two dimensional array of slope.
    """
    dz_dy, dz_dx = np.gradient(dem)
    return np.rad2deg(np.arctan(np.sqrt(dz_dx ** 2.0 + dz_dy ** 2.0)))
...
def gradSlopeAspect(dem):
    ...
    # Loop though and deal with all of the conditions to convert the arctan
    # output to azimuth in degrees relative to North.
    for i in range(0, len(dz_dx)):
        ...
        # y-axis and 0-360.
        if (dz_dx[i]) > 0:
            aspect[i] = 270.0 - aspect[i]
            continue
        else:
            aspect[i] = 90.0 - aspect[i]
            continue
